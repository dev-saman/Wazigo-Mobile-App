# Wazigo Mobile App — Handover (end of Stage 12)
...
| 13. Realtime (Reverb) with REST fallback | **next** |
| 14–15 | not started (see plan) |

Checks currently passing: `npm run lint`, `npm run typecheck`, `npm test` (161 tests),
...
## 7. Next: Stage 13 (realtime with REST fallback)

Live updates over Reverb, with the REST fallback staying in place underneath.

1. **Three answers are needed before any of it can run**: the `/broadcasting/auth` location
   (open question 2), the Reverb app key (question 3), and `tenant_id`, which only exists as a JWT
   claim (question 6 in the plan) but is needed for the channel name.
2. **Backend blocker 5 decides whether to subscribe at all.** The current channel
   `private-tenant.<tenant>.number.<number>` broadcasts other assignees' message content, which
   this app must not receive. Until that is fixed, keep the REST fallback and do not subscribe.
3. A Pusher-compatible client has to be chosen and **verified against RN 0.86 / SDK 57** — it is a
   native dependency, so it belongs with the device build, not after it.
4. Keep the socket behind `src/services/socket/` and an app-level interface (the rest of the app
   must not import the library), and register its teardown with `registerSessionCleanup` so logout
   and expiry close it.
5. The REST fallback to build out: refresh the chats list and the open thread on app foreground
   (the reply window already recomputes there), plus the **bootstrap permission re-fetch deferred
   from Stage 5**.
6. Presence (CHAT-16 `PATCH /me/presence`, CHAT-17 heartbeat every 60 s while foregrounded) belongs
   in this stage; the `presence` slice is still unregistered.

Stages 14–15 remain as listed in development-plan.md.
