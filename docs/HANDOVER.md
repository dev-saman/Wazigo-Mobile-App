# Wazigo Mobile App — Handover (end of Stage 4)
...
Checks currently passing: `npm run lint`, `npm run typecheck`, `npm test` (39 tests),
...
## 7. Next: Stage 5 (bootstrap, permissions, session restore)

`restoreSession()` currently trusts the stored tokens and leaves `auth.user` null until a
screen needs it. Stage 5 completes it:

1. `bootstrap` slice + thunk around AUTH-05 `GET /me/bootstrap`, using **user, roles,
   permissions and numbers only** (ignore routes/menus/flags/settings/billing).
2. Call it from `restoreSession()` (before flipping to `authenticated`, so the app never
   renders without permissions) and after every successful login in `startSession`.
3. A `usePermission()` / `selectHasPermission(key)` helper over `Permissions` in
   `src/api/types.ts`; screens gate on `conversations.view`, `dashboard.view`, etc.
4. Access Denied (design screen 14) via the existing `StateView` when a permission is missing.
5. Bootstrap failure handling: 401/403 -> sign out; offline or 5xx -> keep the session and
   let the screen retry (do not bounce the user to Login for a bad connection).
6. Replace the `(app)/index.tsx` placeholder once the dashboard lands in Stage 6.
