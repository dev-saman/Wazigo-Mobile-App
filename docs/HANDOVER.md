# Wazigo Mobile App — Handover (end of Stage 15)
...
| Next | **the first device pass** (§7) |

Checks currently passing: `npm run lint`, `npm run typecheck`, `npm test` (182 tests),
`npx expo-doctor` (21/21), `npx expo export` for iOS + Android, and route types generated by
`npm start` - all re-run from a fresh clone with `npm ci` at the end of Stage 15.
`npm audit`: 14 moderate, 0 high/critical - read §3 before touching them.
...
## 7. Next: the first device pass

Every phase-1 screen is built and the repo is ready to hand over. What is left cannot be done from
here.

1. **Run it on a device or emulator** — `npm run android` / `npm run ios`. Nothing has run on
   either yet. Expect the first session to be about fonts, safe areas, the keyboard, the image
   picker, SecureStore and the offline strip, rather than about logic.
2. **Make the first real login** against the production API, and confirm the OTP length
   (question 5). A `test_account` would help.
3. **Send [backend-blockers.md](backend-blockers.md) to the server team.** Blockers 1-3 and 6 are
   what make "My Chats" personal; blocker 5 and question 3 decide whether a socket is ever
   connected.
4. **Decide media viewing** (question 7) before the first store build: opening documents or playing
   video and audio needs a native dependency.
5. **Keep an eye on `npm audit`** when Expo ships SDK 57 patches (see Stage 15 in §3); never
   `npm audit fix --force`.
