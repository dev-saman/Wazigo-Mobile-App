# Wazigo Mobile App — Handover (end of Stage 6)
...
Checks currently passing: `npm run lint`, `npm run typecheck`, `npm test` (58 tests),
...
## 7. Next: Stage 7 (chats list, search, filters, pagination)

Design screen 5, CHAT-01, gated on `conversations.view`. Replaces `(tabs)/chats.tsx`.

1. `conversations` slice + thunk: page state, `per_page`, `last_page`, merge-by-id so a refresh
   never duplicates rows, and a separate `refreshing` flag from `loadingMore`.
2. **Default the list to `assigned=mine`.** It is a server-side filter, so it is the closest the
   API comes to the personal scope this app promises - unlike client-side filtering, which is
   never the fix. Keep "Unassigned" as an explicit chip, since CHAT-01 currently returns own +
   unassigned anyway (**backend blocker 1**).
3. Filter chips: All / Open / Unread / Priority (the design's "Awaiting" has no matching filter).
   Search is `search=` on the server, debounced ~350 ms, and cancelled on each keystroke via the
   request `signal` - never filtered locally.
4. Row: avatar with initials, contact name, last-message preview with direction/status, relative
   time in Asia/Kolkata, unread badge, priority marker, and a hint when the 24-hour window is closed.
5. List: `FlatList` unless FlashList is confirmed stable on SDK 57 / RN 0.86. `onEndReached` loads
   the next page only while `current_page < last_page`; guard against duplicate fetches.
6. Empty state = design screen 11 **without** "Start a Conversation" (no outbound creation API).
   Offline and error states reuse `StateView`, as the dashboard does.
7. Mark-as-read (CHAT-06) belongs to Stage 8 when the thread opens, not to the list.

Stages 8–15 remain as listed in development-plan.md.
